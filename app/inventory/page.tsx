  ...
  return (
    <div className="flex items-center justify-between border-t border-line py-2 first:border-0">
      ...
      <div className="flex items-center gap-4 text-right">
        ...
        <span className="w-20 text-[12px] text-positive tabular">{money(pool.valueRemaining)}</span>
      </div>
    </div>
  );
}
...
export default async function InventoryPage() {
  ...
  // Group pouches by SKU; list each facility inside the SKU's card.
            ...
            <Card>
              <div className="mb-1 flex items-center gap-3">
                {teabags[0]?.imageUrl ? (
                  ...
                  <div className="flex h-11 w-11 items-center justify-center rounded-[12px] bg-accent-soft text-xl">🍵</div>
                )}
                <div className="flex-1">
                  <div className="font-semibold text-ink">Tea bags</div>
                  <div className="text-[12px] text-muted">One material, stocked across facilities</div>
                </div>
                <div className="text-right">
                  <div className="font-semibold text-ink tabular">{qty(teabagUnits)}</div>
                  <div className="text-[12px] text-positive tabular">{money(teabagValue)}</div>
                </div>
              </div>
              <div className="mt-2">
                {teabags.map((p) => (
            ...
            <Card key={g.sku}>
              <div className="mb-1 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate font-semibold text-ink">{g.productName}</div>
                  <div className="text-[12px] text-muted">{g.facilities.length === 1 ? "1 facility" : `${g.facilities.length} facilities`}</div>
                </div>
                <div className="text-right">
                  <div className="font-semibold text-ink tabular">{qty(g.totalQty)}</div>
                  <div className="text-[12px] text-positive tabular">{money(g.totalValue)}</div>
                </div>
              </div>
              <div className="mt-2">
                {g.facilities.map((p) => (
