...
import { LTV_HORIZONS, buildLtvReport, decodeLtvOrders, type LtvCell, type LtvHorizon } from "@/lib/ltv";
import type { LtvPayload } from "@/lib/ltv-data";

type Metric = "profit" | "revenue";
const AGE_LABEL: Record<LtvHorizon, string> = { 0: "First order", 30: "30 days", 60: "60 days", 90: "90 days", 180: "180 days", 365: "1 year", all: "All time" };
const CHANNELS = [
...
export function LtvClient({ data }: { data: LtvPayload }) {
  ...
  // Which customers: the ones who FIRST ordered in this range. Everything they bought afterwards
  // still counts toward their value.
  const [range, setRange] = useState<Range>({ key: "all", from: data.oldest, to: data.newest });
  const orders = useMemo(() => decodeLtvOrders(data.wire), [data.wire]);
  const now = useMemo(() => new Date(data.computedAt).getTime(), [data.computedAt]);
  const r = useMemo(() => {
    const b = range.key === "all" ? { from: data.oldest, to: data.newest } : rangeBounds(range.key, data.newest, range.from, range.to);
    return buildLtvReport({ orders, adSpendByMonth: data.adSpendByMonth, now, firstOrderFrom: b.from ?? data.oldest, firstOrderTo: b.to ?? data.newest });
  }, [orders, data.adSpendByMonth, data.oldest, data.newest, now, range]);
  ...
  const word = metric === "profit" ? "Profit" : "Revenue";
  ...
  if (!data.connected || !data.customerAccess || data.wire.orders.length === 0) {
    ...
    return (
      <div className="flex flex-col gap-5">
        <div className="flex flex-wrap items-center gap-2">{channelTabs}</div>
  ...
  // One shade scale for the whole table, that one unusual customer cannot flatten: the 90th percentile.
  ...
  const ageCell = (cell: LtvCell | undefined, total: number, key: string, solid = false) => {
    const v = value(cell);
    if (!cell || v == null) return <td key={key} className="px-3 py-2 text-right text-muted">—</td>;
    const strength = Math.max(0, Math.min(1, v / peak));
    const partial = !cell.complete && !solid;
    const label = AGE_LABEL[cell.horizon].toLowerCase();
    return (
      <td key={key} className="px-1 py-1 text-right" title={cell.horizon === "all" ? `Everything these ${cell.customers} customers have ordered so far` : cell.complete ? `${cell.customers} customers` : `${cell.customers} of ${total} customers are old enough so far; the rest haven't reached ${label} yet`}>
        <span className={`inline-block w-full rounded-md px-2 py-1 tabular ${v < 0 ? "text-negative" : "text-ink"} ${partial ? "opacity-60" : ""}`} style={{ background: v > 0 ? `color-mix(in srgb, var(--color-accent) ${Math.round(6 + strength * 26)}%, transparent)` : undefined }}>
          {money(v)}
          {partial && <span className="ml-1 text-[10px] text-muted">·</span>}
        </span>
      </td>
    );
  };

  const curve = r.overall.filter((c) => c.horizon !== "all" && value(c) != null);
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center gap-2">
        {channelTabs}
        <span className="ml-1 text-[12px] text-muted">Customers who first ordered in</span>
        <DateRangePicker value={range} onChange={(next) => setRange({ key: next.key, from: next.from || data.oldest, to: next.to || data.newest })} newest={data.newest} oldest={data.oldest} locale={locale} />
      ...
      <p className="-mt-2 text-[12px] text-muted">
        {metric === "profit" ? "Profit: after what the units really cost and the order's own fees. Ads are shown apart, as what a new customer cost." : "Revenue: what customers paid for goods and shipping, after discounts and refunds. Tax left out."} Whatever a customer bought after the chosen dates still counts toward their value.
      </p>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Customers" value={r.customers.toLocaleString(locale)} sub={`${r.orders.toLocaleString(locale)} orders · ${r.ordersPerCustomer} each`} />
        ...
        <StatCard label={`${word} per customer · 90 days`} value={show(value(at(r.overall, 90)))} sub={`first order ${show(value(at(r.overall, 0)))} · ${at(r.overall, 90)?.customers ?? 0} customers old enough`} />
        <StatCard label={`${word} per customer · all time`} value={show(value(at(r.overall, "all")))} sub="everything each customer has ordered so far" />
      </div>

      {r.customers === 0 ? (
        <Card><p className="text-[13px] text-muted">No customer placed their first order in these dates. Pick a wider range.</p></Card>
      ) : (
        <>
          <Card padded={false} className="overflow-hidden">
            ...
            <div className="mt-3 overflow-x-auto">
              <table className={`w-full text-[12.5px] ${hasAds ? "min-w-[1060px]" : "min-w-[840px]"}`}>
                <thead>
                  <tr className="border-y border-line text-[11.5px] text-muted">
                    ...
                    {LTV_HORIZONS.map((h) => <th key={h} className="px-3 py-2 text-right font-medium" title={h === "all" ? "Everything each customer has ordered so far. Older customers have had longer to get there." : undefined}>{AGE_LABEL[h]}</th>)}
                ...
                <tbody>
                  <tr className="border-b border-line bg-surface-2/50 font-medium">
                    ...
                    {LTV_HORIZONS.map((h) => ageCell(at(r.overall, h), r.customers, String(h), true))}
                  ...
                  {[...r.cohorts].reverse().map((c) => (
                    <tr key={c.month} className="border-b border-line last:border-0">
                      ...
                      {LTV_HORIZONS.map((h) => ageCell(at(c.cells, h), c.customers, String(h)))}
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,5fr)_minmax(0,6fr)]">
            <Card>
              ...
              {curve.length > 1 ? (
                ...
                <p className="mt-6 text-[12.5px] text-muted">Not enough history in these dates yet: this fills in as these customers get older.</p>
              )}
            </Card>
                ...
                <table className="w-full text-[12.5px]">
                  <thead>
                    <tr className="border-y border-line text-[11.5px] text-muted">
                      ...
                      <th className="px-5 py-2 text-right font-medium">All time</th>
                    </tr>
                  </thead>
                  <tbody>
                    {r.byFirstProduct.map((p) => (
                      <tr key={p.product} className="border-b border-line last:border-0">
                        ...
                        {([0, 90, "all"] as LtvHorizon[]).map((h, i) => {
                          const cell = at(p.cells, h);
                          const v = value(cell);
                          return <td key={String(h)} className={`${i === 2 ? "px-5" : "px-3"} py-2 text-right tabular ${v == null ? "text-muted" : v < 0 ? "text-negative" : "text-ink"} ${cell && !cell.complete ? "opacity-60" : ""}`} title={cell && v != null && h !== "all" ? `${cell.customers} of ${p.customers} customers old enough` : undefined}>{show(v)}</td>;
                        })}
                      </tr>
                    ))}
                  </tbody>
      ...
      <div className="flex flex-col gap-1 text-[11.5px] text-muted">
        {data.wholesale.orders > 0 && <p>{data.wholesale.orders.toLocaleString(locale)} wholesale orders ({data.wholesale.sources.join(", ")}) are left out of this page: a retailer buying to resell isn&apos;t a customer&apos;s lifetime value. They still count on the P&L.</p>}
        {!hasAds && r.customers > 0 && <p>No ad spend is counted on this channel, so what a new customer cost in ads isn&apos;t shown. It appears here as soon as an ad platform&apos;s spend counts against Shopify (Settings, Integrations).</p>}
        {data.ordersWithoutCustomer > 0 && <p>{data.ordersWithoutCustomer.toLocaleString(locale)} orders carry no customer in Shopify and are left out of this page. They still count on the P&L.</p>}
      </div>
    </div>
  );
}
